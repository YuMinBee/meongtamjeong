...
이 저장소는 발표용 경량 API 구조를 담고 있으며, 전체 `dog` 프로젝트 기준으로는 아래 확장이 추가로 진행되었습니다.

- `CLIP + FAISS` 기반 텍스트·이미지 유사도 검색
- `Gemma 3` 기반 추천 문장 생성
- VLM 기반 공고 설명 보강 완료
- 보호소 원문 설명과 VLM 보강 설명을 합친 `merged_desc` 생성
- 종 코드별 공고 갤러리와 검색 평가 리포트 생성

## 향후 발전 방향

- 공고 텍스트와 이미지 임베딩을 함께 활용하는 멀티모달 검색 고도화
- 사용자 설문 기반 추천 가중치 조정
- 보호소·공공기관 데이터베이스와 연동
