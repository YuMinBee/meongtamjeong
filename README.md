...
| 우선순위 | 방향 | 내용 | 기대 효과 |
| --- | --- | --- | --- |
| 1 | 입양 상담 흐름 고도화 | 거주 형태, 가족 구성, 산책 가능 시간, 알레르기, 첫 반려견 여부를 단계적으로 질문하고 검색 질의로 변환 | 사용자가 막연한 문장 대신 구체적인 조건으로 후보를 탐색 가능 |
| 2 | 추천 근거 설명 강화 | 후보별로 크기, 나이, 성별, 설명, 유사도 점수를 바탕으로 왜 추천됐는지 짧게 제시 | 추천 결과에 대한 신뢰도와 해석 가능성 향상 |
| 3 | VLM 공고 작성 보조 기능 | 사진과 기존 설명을 바탕으로 보호소 담당자가 바로 쓸 수 있는 공고 설명 초안 생성 | 보호소의 공고 작성 부담 감소, 공고 정보 품질 개선 |
| 4 | 사진 품질 점검 | 흐림, 어두움, 얼굴 가림, 개체가 너무 작게 나온 사진을 감지하고 재촬영 필요 여부 안내 | 온라인 공고의 첫인상과 탐색 효율 개선 |
| 5 | 피드백 기반 재랭킹 | 사용자의 "관심 있음/없음" 반응을 저장해 다음 검색에서 선호 조건을 반영 | 개인화 추천 품질 개선 |
| 6 | 보호소 관리자 대시보드 | 설명 부족 공고, 사진 없는 공고, 오래된 공고, 종 코드별 공고 수를 한 화면에서 확인 | 보호소 운영 효율과 데이터 관리 품질 향상 |
| 7 | 유기묘·타 반려동물 확장 | 동물 분류와 메타 필드를 확장해 유기묘 등 다른 보호동물에도 적용 | 시스템 확장성 검증 |

### 추천하는 다음 구현 순서

1. **입양 상담 스킬**: 설문 입력을 자연어 검색 질의로 바꾸는 기능을 먼저 고도화합니다. 현재 설문 기반 추천 API가 있으므로 가장 적은 변경으로 사용자 경험을 개선할 수 있습니다.
2. **추천 근거 설명**: Top-K 결과마다 추천 이유를 구조화해 반환합니다. 발표나 시연에서 "왜 이 후보인가"를 설명하기 쉬워집니다.
3. **보호소 공고 보강 도구**: 이미 구현된 VLM 설명 보강을 보호소 관리자용 기능으로 정리합니다. 사진 기반 설명 초안, 주의 문구, 공고 품질 점검을 함께 묶으면 서비스 가치가 더 명확해집니다.
