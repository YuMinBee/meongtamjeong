...
- [x] Git 전체 reachable 128,300,529바이트 비밀검사 PASS(누락 blob 0, 실제값 비출력); 과거 예시 전용 `.env.example` 1건은 정확 일치 검토 allowlist로 기록
...
- [x] [공식 결과보고서 양식 기반 DOCX/PDF 초안 및 SBOM·AI 명세 붙임 생성](../submission/2026-osscontest/README.md)
- [ ] 3분 이내 시연영상
- [ ] 공개 소스코드와 릴리스 태그
- [x] README 빠른 시작
...
- [x] 깨끗한 검증 환경의 전체 설치 패키지 75개를 SPDX 2.3·CycloneDX 1.6 SBOM으로 변환하고 직접 선언 관계 25개만 dependency edge로 기록
...
- 공식 포털 대비 실제 후보 선택 파일럿, 시연영상, 접수 정보 입력, 릴리스 태그가 아직 남아 있습니다.
